    ...
    >
      ...
      <template #optional="{ record }">
        <a-space>
          ...
          <a-button
            type="primary"
            v-if="activeInfo.users && !activeInfo.users.includes(record.id)"
            @click="addActiveQuestion(record.id)"
          >
            添加到活动
          </a-button>
...
const addActiveQuestion = async (questionId: number) => {
  const res = await ActiveControllerService.addActiveUserUsingGet(
    activeInfo.value?.id,
    questionId
  );
  if (res.code === 0) {
